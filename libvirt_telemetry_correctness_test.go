...
func TestCPUSeverityUsesDelayWithWaitAsNonAdditiveFallback(t *testing.T) {
	...
	tests := []struct {
		...
		wantResourceSource string
	}{
		{
			name:          "both counters prefer delay without adding wait",
			previousDelay: 100_000_000, currentDelay: 150_000_000, delayPresent: true,
			previousWait: 100_000_000, currentWait: 600_000_000, waitPresent: true,
			wantPressure: 0.2, wantResourceSource: "usage,steal",
		},
		{
			name:          "delay only",
			previousDelay: 100_000_000, currentDelay: 250_000_000, delayPresent: true,
			wantPressure: 0.6, wantResourceSource: "usage,steal",
		},
		{
			name:         "wait fallback",
			previousWait: 100_000_000, currentWait: 300_000_000, waitPresent: true,
			wantPressure: 0.8, wantResourceSource: "usage,wait",
		},
		{
			name:          "delay reset falls back to valid wait",
			previousDelay: 200_000_000, currentDelay: 100_000_000, delayPresent: true,
			previousWait: 100_000_000, currentWait: 200_000_000, waitPresent: true,
			wantPressure: 0.4, wantResourceSource: "usage,wait",
		},
	}
	...
	for index, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			uuid := "vm-cpu-scheduler-delay-" + string(rune('a'+index))
			shard := shardIndex(uuid)
			mc.im.cpuSamples[shard][uuid] = cpuSample{
				total:        1_000_000_000,
				steal:        test.previousDelay,
				wait:         test.previousWait,
				vcpuCount:    1,
				stealPresent: test.delayPresent,
				waitPresent:  test.waitPresent,
			...
			stat := &ParsedStats{
				...
				Vcpus: map[int]*VcpuStat{
					0: {
						Delay:        test.currentDelay,
						DelayPresent: test.delayPresent,
						Wait:         test.currentWait,
						WaitPresent:  test.waitPresent,
					},
				},
			}
			metrics := make([]prometheus.Metric, 0, 4)
			pressure, available, sources := mc.collectDomainCPUMetricsWithSources(
				stat,
				now,
			...
			if !available || math.Abs(pressure-test.wantPressure) > 1e-9 {
				t.Fatalf("pressure=%v available=%v, want %v/true", pressure, available, test.wantPressure)
			}
			if got := strings.Join(sources, ","); got != test.wantResourceSource {
				t.Fatalf("sources=%q, want %q", got, test.wantResourceSource)
			}
		})
	}
}
