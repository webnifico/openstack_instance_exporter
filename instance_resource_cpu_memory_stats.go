...
func (mc *MetricsCollector) collectDomainMemoryMetrics(
	...
	guestUsedMB, resourceMemSeverity, guestUsedAvailable, memPressureAvailable, _ := mc.collectDomainMemoryMetricsWithSources(
		stat,
		now,
	...
	return guestUsedMB, resourceMemSeverity, guestUsedAvailable, memPressureAvailable
}

func (mc *MetricsCollector) collectDomainMemoryMetricsWithSources(
	stat *ParsedStats,
	now time.Time,
	domain, serverName, instanceUUID, projectUUID, projectName, userUUID string,
	instanceRunning bool,
	...
	telemetryFresh bool,
	dynamicMetrics *[]prometheus.Metric,
) (float64, float64, bool, bool, []string) {
	...
	resourceSources := make([]string, 0, 3)
	if guestUsedAvailable {
		resourceSources = append(resourceSources, "guest_used")
	}
	if swapInRateAvailable {
		resourceSources = append(resourceSources, "swap_in")
	}
	if majorRateAvailable {
		resourceSources = append(resourceSources, "major_fault")
	}

	resourceMemSeverity := math.Max(memUsageSeverity, math.Max(swapSeverity, majorSeverity))
	return guestUsedMB, resourceMemSeverity, guestUsedAvailable, len(resourceSources) > 0, resourceSources
}
...
func (mc *MetricsCollector) collectDomainCPUMetrics(
	...
	pressure, available, _ := mc.collectDomainCPUMetricsWithSources(
		stat,
		time.Now(),
		domain, serverName, instanceUUID, projectUUID, projectName, userUUID,
		vcpuCount,
	...
	return pressure, available
}

func (mc *MetricsCollector) collectDomainCPUMetricsWithSources(
	stat *ParsedStats,
	now time.Time,
	domain, serverName, instanceUUID, projectUUID, projectName, userUUID string,
	vcpuCount int,
	...
	cpuUsage, stealPercent, waitPercent, usageValid, stealValid, waitValid := mc.im.calculateCPUUsageWithDetailedAvailabilityAt(
		stat.CpuTime,
		stealTotal,
		waitTotal,
		stealAvailable,
	...
	*dynamicMetrics = append(*dynamicMetrics, prometheus.MustNewConstMetric(mc.im.instanceCpuVcpuPercentDesc, prometheus.GaugeValue, roundToFiveDecimals(cpuUsage), domain, serverName, instanceUUID, projectUUID, projectName, userUUID))
	...
	stall01 := 0.0
	resourceSources := []string{"usage"}
	if stealValid {
		// Libvirt's delay and wait counters both describe time waiting in the
		// host scheduler runqueue. Prefer delay (the guest-visible steal-time
		// source) and use wait only as a compatibility fallback; adding the two
		// aliases would double-count scheduler contention.
		stall01 = clamp01(stealPercent / 25.0)
		resourceSources = append(resourceSources, "steal")
	} else if waitValid {
		stall01 = clamp01(waitPercent / 25.0)
		resourceSources = append(resourceSources, "wait")
	}
	return math.Max(stall01, usage01*0.3), true, resourceSources
}
