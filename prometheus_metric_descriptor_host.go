...
func initHostMetrics(mc *MetricsCollector) {
	...
	descs := []d{
		{dst: &mc.hostMemTotalMBDesc, name: "oie_host_mem_mb_total", help: "Total physical memory on this hypervisor in mebibytes; metric name retained for compatibility"},
		...
		{dst: &mc.hostCollectionIntervalSecondsDesc, name: "oie_host_collection_interval_seconds", help: "Effective background collection interval on this host (seconds)"},
		...
		{dst: &mc.hostLibvirtOkDesc, name: "oie_host_libvirt_ok", help: "1 if the most recent Libvirt domain collection completed successfully, else 0"},
		{dst: &mc.hostLibvirtLastSuccessTimestampDesc, name: "oie_host_libvirt_last_success_timestamp_seconds", help: "Unix timestamp of the last successful complete Libvirt domain collection (seconds)"},
		{dst: &mc.hostLibvirtStaleSecondsDesc, name: "oie_host_libvirt_stale_seconds", help: "Seconds since the last successful complete Libvirt domain collection; -1 before the first success"},
		...
		{dst: &mc.hostConntrackStaleSecondsDesc, name: "oie_host_conntrack_stale_seconds", help: "Seconds since the last successful conntrack read; -1 before the first success"},
		...
		{dst: &mc.hostMemFreeMBDesc, name: "oie_host_mem_free_mb", help: "Free host memory (MemFree) in mebibytes; metric name retained for compatibility"},
		{dst: &mc.hostMemAvailableMBDesc, name: "oie_host_mem_available_mb", help: "Available host memory (MemAvailable) in mebibytes; metric name retained for compatibility"},
	}
	for _, d := range descs {
		*d.dst = newHostMetricDesc(d.name, d.help)
	}
	mc.hostVolumeRetypeResultsTotalDesc = prometheus.NewDesc(
		"oie_host_volume_retype_results_total",
		"Process-local Libvirt-observed terminal RBD block-copy results on this host: success, unsuccessful, or unknown according to whether the final live source matched the saved destination, original source, or neither; not authoritative Cinder outcomes",
		[]string{"result"},
		nil,
	)
}
