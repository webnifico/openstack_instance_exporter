...
func TestHighConfidenceMiningUsesMiningSpecificEvidence(t *testing.T) {
	...
	feature.MiningHigh.TopPairFlows = 2
	feature.MiningHigh.TopPairReplied = 2
...
func TestSharedMiningPortsRequireStrongerEvidence(t *testing.T) {
	...
	feature.MiningShared.TopPairFlows = 3
...
func TestSharedMiningPortsRequireReplyEvidenceAndConcentration(t *testing.T) {
	...
	base.MiningShared.TopPairFlows = 1
	base.MiningShared.TopPairReplied = 1
...
func TestMiningConfidencePromotionRequiresRepliesOnSelectedPair(t *testing.T) {
	remoteA := IPStrToKey("198.51.100.70")
	remoteB := IPStrToKey("198.51.100.71")

	highStats := newBehaviorStats(false)
	highStats.updateOutboundMining(remoteA, 10128, 6, IPS_SEEN_REPLY, true)
	for i := 0; i < 4; i++ {
		highStats.updateOutboundMining(remoteA, 10128, 6, 0, true)
	}
	highStats.updateOutboundMining(remoteB, 10128, 6, IPS_SEEN_REPLY, true)
	high, _ := highStats.summarizeMining(nil)
	if high.TopPairFlows != 5 || high.TopPairReplied != 1 {
		t.Fatalf("dedicated selected pair=%d/%d, want 5 flows/1 replied: %+v", high.TopPairFlows, high.TopPairReplied, high)
	}
	if got := selectMiningDetectionEvidence(BehaviorFeature{Direction: "outbound", MiningHigh: high}, newBehaviorScaler(1)); !got.Valid || got.Confidence != miningPortConfidenceHighPersistent {
		t.Fatalf("dispersed dedicated replies promoted selected pair: %+v", got)
	}

	sharedStats := newBehaviorStats(false)
	sharedStats.updateOutboundMining(remoteA, 7777, 6, IPS_SEEN_REPLY, true)
	for i := 0; i < 2; i++ {
		sharedStats.updateOutboundMining(remoteA, 7777, 6, 0, true)
	}
	sharedStats.updateOutboundMining(remoteB, 7777, 6, IPS_SEEN_REPLY, true)
	_, shared := sharedStats.summarizeMining(nil)
	if shared.TopPairFlows != 3 || shared.TopPairReplied != 1 {
		t.Fatalf("shared selected pair=%d/%d, want 3 flows/1 replied: %+v", shared.TopPairFlows, shared.TopPairReplied, shared)
	}
	if got := selectMiningDetectionEvidence(BehaviorFeature{Direction: "outbound", MiningShared: shared}, newBehaviorScaler(1)); !got.Valid || got.Confidence != miningPortConfidenceSharedPersistent {
		t.Fatalf("dispersed shared replies promoted selected pair: %+v", got)
	}
}

func TestMiningPairPromotionDoesNotSkipDominantEndpoint(t *testing.T) {
	noisyRemote := IPStrToKey("198.51.100.72")
	stableRemote := IPStrToKey("198.51.100.73")
	stats := newBehaviorStats(false)
	stats.updateOutboundMining(noisyRemote, 10128, 6, IPS_SEEN_REPLY, true)
	for i := 0; i < 20; i++ {
		stats.updateOutboundMining(noisyRemote, 10128, 6, 0, true)
	}
	stats.updateOutboundMining(stableRemote, 10128, 6, IPS_SEEN_REPLY, true)
	stats.updateOutboundMining(stableRemote, 10128, 6, IPS_ASSURED, true)

	high, _ := stats.summarizeMining(nil)
	if high.TopRemote != noisyRemote || high.TopPairFlows != 21 || high.TopPairReplied != 1 {
		t.Fatalf("selected pair=%s %d/%d, want dominant endpoint: %+v",
			IPKeyToString(high.TopRemote), high.TopPairFlows, high.TopPairReplied, high)
	}
	if got := selectMiningDetectionEvidence(BehaviorFeature{Direction: "outbound", MiningHigh: high}, newBehaviorScaler(1)); !got.Valid || got.Confidence != miningPortConfidenceHighPersistent {
		t.Fatalf("secondary replies promoted the dominant endpoint: %+v", got)
	}
}
