...
func TestDiskMetricCollectionDeduplicationFieldSemanticsAndLatency(t *testing.T) {
	mc := newCollectorOrchestrationTestCollector(t)
	...
	allocationPaths := make([]string, 0)
	for _, metric := range firstMetrics {
		match := descNameRE.FindStringSubmatch(metric.Desc().String())
		if len(match) != 2 || match[1] != "oie_instance_disk_allocation_bytes" {
			continue
		}
		labels := dataIntegrityMetricLabels(metricDTO(t, metric))
		allocationPaths = append(allocationPaths, labels)
		if !strings.Contains(metric.Desc().String(), "offset of the highest written sector") ||
			!strings.Contains(metric.Desc().String(), "not backend physical usage") {
			t.Fatalf("allocation metric help is misleading: %s", metric.Desc())
		}
	}
	if len(allocationPaths) != 1 || !strings.Contains(allocationPaths[0], `disk_path="vdc"`) {
		t.Fatalf("allocation metrics=%v, want only the disk with a real Libvirt allocation field", allocationPaths)
	}
...
func TestCumulativeByteCountersPreserveSubFiveDecimalGibibytePrecision(t *testing.T) {
	mc := newCollectorOrchestrationTestCollector(t)
	now := time.Unix(1_700_000_100, 0)

	diskMetrics := make([]prometheus.Metric, 0)
	mc.collectDomainDiskMetrics(
		&DomainStatic{Disks: []DomainDisk{{TargetDev: "vda", Type: "network", SourceName: "pool/volume"}}},
		&ParsedStats{Disks: map[int]*DiskStat{0: {
			Name: "vda", NamePresent: true,
			RdBytes: 1, RdBytesPresent: true,
			WrBytes: 2, WrBytesPresent: true,
		}}},
		now,
		"domain", "server", "vm-byte-precision-disk", "project", "project-name", "user",
		true,
		&diskMetrics,
	)
	for name, want := range map[string]float64{
		"oie_instance_disk_read_gbytes_total":  bytesToGigabytes,
		"oie_instance_disk_write_gbytes_total": 2 * bytesToGigabytes,
	} {
		got, ok := dataIntegrityMetricFamilyValue(t, diskMetrics, name)
		if !ok || got != want {
			t.Fatalf("%s=%v available=%v, want the unrounded cumulative value %v", name, got, ok, want)
		}
	}

	networkMetrics := make([]prometheus.Metric, 0)
	mc.collectDomainNetworkAndConntrack(
		&DomainStatic{Interfaces: []string{"tap0"}},
		&ParsedStats{Nets: map[int]*NetStat{0: {
			Name: "tap0", NamePresent: true,
			RxBytes: 1, RxBytesPresent: true,
			TxBytes: 2, TxBytesPresent: true,
		}}},
		now,
		"domain", "server", "vm-byte-precision-network", "project", "project-name", "user",
		false,
		nil,
		nil,
		nil,
	...
	for name, want := range map[string]float64{
		"oie_instance_net_rx_gbytes_total": bytesToGigabytes,
		"oie_instance_net_tx_gbytes_total": 2 * bytesToGigabytes,
	} {
		got, ok := dataIntegrityMetricFamilyValue(t, networkMetrics, name)
		if !ok || got != want {
			t.Fatalf("%s=%v available=%v, want the unrounded cumulative value %v", name, got, ok, want)
		}
	}
