...
type hostResourceSnapshot struct {
	...
	totalMemAvailable bool
	cpuPercent        float64
	cpuAvailable      bool
	memFreeMB         float64
	memAvailableMB    float64
	memFreeAvailable  bool
	memAvailAvailable bool
}

func (mc *MetricsCollector) readHostResourceSnapshot() hostResourceSnapshot {
	if mc != nil && mc.hostResourceSnapshotOverride != nil {
		return mc.hostResourceSnapshotOverride()
	}
	...
	return hostResourceSnapshot{
		...
		totalMemAvailable: totalMemAvailable,
		cpuPercent:        cpuPercent,
		cpuAvailable:      cpuAvailable,
		memFreeMB:         memFreeMB,
		memAvailableMB:    memAvailableMB,
		memFreeAvailable:  memFreeAvailable,
		memAvailAvailable: memAvailAvailable,
	}
}
