...
const maxCombinedThreatHitsPerInstance = 5000
...
func (cm *ConntrackManager) newConntrackAggregator(
	...
	agg := &ConntrackAgg{
		...
		CombinedThreatHits:        make(map[string]map[PairKey]ConntrackEntry),
		CombinedThreatHitsDropped: make(map[string]uint64),
	...
	agg.SpamhausSourceIncluded = spamEnabled
	...
	combinedHitHeaps := make(map[string]*threatPairKeyMaxHeap)
	...
	addHit := func(dst map[string]map[PairKey]ConntrackEntry, inst string, k PairKey, ct ConntrackEntry) {
		...
		if retainCappedThreatHit(h, retained, k, ct, maxSpamhausHitsPerInstance) {
			agg.SpamhausHitsDropped[inst] = 1
		}
	}
	...
	addCombinedHit := func(inst string, k PairKey, ct ConntrackEntry) {
		hits := agg.CombinedThreatHits[inst]
		if hits == nil {
			hits = make(map[PairKey]ConntrackEntry, 16)
			agg.CombinedThreatHits[inst] = hits
		}
		retained := combinedHitHeaps[inst]
		if retained == nil {
			fresh := make(threatPairKeyMaxHeap, 0, maxCombinedThreatHitsPerInstance)
			retained = &fresh
			combinedHitHeaps[inst] = retained
		}
		if retainCappedThreatHit(hits, retained, k, ct, maxCombinedThreatHitsPerInstance) {
			// PairKey can recur in raw conntrack input and an evicted key is no
			// longer available for exact membership checks. Record bounded
			// overflow presence instead of an over-countable tail cardinality.
			agg.CombinedThreatHitsDropped[inst] = 1
		}
	}
	...
	consumeOne := func(flow ConntrackFlowLite) {
		...
		var combinedInstances [2]string
		var combinedEvidence [2]ConntrackEntry
		combinedCount := 0
		markCombined := func(inst string, ct ConntrackEntry) {
			for index := 0; index < combinedCount; index++ {
				if combinedInstances[index] == inst {
					return
				}
			}
			if combinedCount < len(combinedInstances) {
				combinedInstances[combinedCount] = inst
				combinedEvidence[combinedCount] = ct
				combinedCount++
			}
		}
		...
		if spamEnabled {
			...
			if spamSrc || spamDst {
				...
				routeThreatInstances(spamDir, instSrc, instDst, spamSrc, spamDst, func(inst string) {
					evidence := getCT()
					addHit(agg.SpamhausHits, inst, k, evidence)
					markCombined(inst, evidence)
				})
			}
		}
		...
		for _, p := range providers {
			...
			routeThreatInstances(p.Dir, instSrc, instDst, inSrc, inDst, func(inst string) {
				evidence := getCT()
				addProviderHit(p.Name, inst, k, evidence)
				markCombined(inst, evidence)
			})
		}

		for index := 0; index < combinedCount; index++ {
			addCombinedHit(combinedInstances[index], k, combinedEvidence[index])
		}
	}
...
func (cm *ConntrackManager) aggregateConntrackOnePassFamilies(
	...
	agg.ObservationUnix = cm.conntrackNow().Unix()
...
func (cm *ConntrackManager) readAndAggregateConntrack(
	...
	dump := func(enabled bool, family int, errp *error) {
		...
		c, p, e, err := cm.dumpConntrackFamily(family, consumeOne)
		count += int(c)
		parseErrs += p
		enobufs += e
		*errp = err
	...
	observationNow := cm.conntrackNow()
	agg.ObservationUnix = observationNow.Unix()
	agg.ObservationTimeSet = true
	atomic.StoreUint64(&cm.conntrackRawOK, 1)
	atomic.StoreInt64(&cm.conntrackLastSuccessUnix, observationNow.Unix())
	cm.storeLastGoodConntrack(agg, count)
	return agg, count, nil
}

func (cm *ConntrackManager) dumpConntrackFamily(family int, consume func(ConntrackFlowLite)) (uint64, uint64, uint64, error) {
	if cm.conntrackDumpFamilyOverride != nil {
		return cm.conntrackDumpFamilyOverride(
			family,
			cm.conntrackRawRcvBufBytes,
			cm.conntrackNetlinkRecvTimeout,
			consume,
		)
	}
	return conntrackDumpFamilyLiteConsume(
		family,
		cm.conntrackRawRcvBufBytes,
		cm.conntrackNetlinkRecvTimeout,
		consume,
...
func (cm *ConntrackManager) conntrackNow() time.Time {
	if cm != nil && cm.conntrackNowOverride != nil {
		return cm.conntrackNowOverride()
	}
	return time.Now()
}
