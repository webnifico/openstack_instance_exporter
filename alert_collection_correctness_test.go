...
func TestNetworkDropPressureDoesNotInheritConntrackAvailabilityConfidence(t *testing.T) {
	...
	mc.im.netSamples[idx] = map[string]netSample{
		instanceUUID: {interfaceSet: resourceNetworkIdentity(mc.im.domainMeta[instanceUUID], deduplicateIPs(mc.im.domainMeta[instanceUUID].FixedIPs)), ts: time.Now().Add(-time.Second)},
	}
