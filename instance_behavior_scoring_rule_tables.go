...
func evalRuleByID(feature BehaviorFeature, sc behaviorScaler, ev BehaviorEvidence, ctx *RuleCtx, table []BehaviorRule, id string) (hit bool, ruleID, kind, reason, source string) {
	for i := range table {
		if table[i].ID == id {
			return evalRules(feature, sc, ev, ctx, table[i:i+1])
		}
	}
	return false, "", "", "", ""
}
