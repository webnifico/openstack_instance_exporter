...
type dataIntegrityE2EMetricCollector struct {
	metrics []prometheus.Metric
}

func (c dataIntegrityE2EMetricCollector) Describe(ch chan<- *prometheus.Desc) {
	prometheus.DescribeByCollect(c, ch)
}

func (c dataIntegrityE2EMetricCollector) Collect(ch chan<- prometheus.Metric) {
	for _, metric := range c.metrics {
		ch <- metric
	}
}

type dataIntegrityE2EBehaviorEvent struct {
	tag          string
	event        string
	domain       string
	instanceUUID string
...
func dataIntegrityE2EUUID() (libvirt.UUID, string) {
	uuid := libvirt.UUID{
		0x10, 0x20, 0x30, 0x40,
		0x50, 0x60,
		0x70, 0x80,
...
func dataIntegrityE2EFixture(t *testing.T) ([]*dto.MetricFamily, []dataIntegrityE2EBehaviorEvent) {
	t.Helper()
	...
	events := make([]dataIntegrityE2EBehaviorEvent, 0, 1)
	mc.cm.LogThreat = func(tag, event, domain, instanceUUID, projectUUID, projectName, userUUID string, kvpairs ...interface{}) {
		...
		events = append(events, dataIntegrityE2EBehaviorEvent{
			tag:          tag,
			event:        event,
			domain:       domain,
	...
	const (
		...
		serverName  = "data-integrity-miner-fixture"
		projectUUID = "project-data-integrity"
		projectName = "Data Integrity Project"
		userUUID    = "user-data-integrity"
	...
	for cycle := 1; cycle <= 3; cycle++ {
		connAgg.ObservationUnix = 100 + int64(cycle-1)*behaviorPersistenceReferenceSeconds
	...
	registry.MustRegister(dataIntegrityE2EMetricCollector{metrics: finalMetrics})
...
func dataIntegrityMetricValue(metric *dto.Metric) (float64, bool) {
	switch {
...
func dataIntegrityMetricLabels(metric *dto.Metric) string {
...
func dataIntegritySelectedMetricContract(t *testing.T, families []*dto.MetricFamily) string {
	t.Helper()
	...
	for _, family := range families {
		...
		value, ok := dataIntegrityMetricValue(family.Metric[0])
		if !ok {
		...
		lines = append(lines, fmt.Sprintf(
			...
			family.GetHelp(),
			dataIntegrityMetricLabels(family.Metric[0]),
...
func dataIntegrityEmittedMetricSchemaContract(families []*dto.MetricFamily) string {
...
func resourceTelemetryMetricFamilyNames(t *testing.T) map[string]struct{} {
	t.Helper()
	names := make(map[string]struct{}, resourceTelemetryOIEFamilyCount-dataIntegrityOIEFamilyCount)
	for _, line := range goldenLines(t, resourceTelemetryOIEAdditionsGoldenPath) {
		name, _, ok := strings.Cut(line, "|")
		if !ok || name == "" {
			t.Fatalf("invalid Resource telemetry metric addition %q", line)
		}
		names[name] = struct{}{}
	}
	return names
...
func alertValidationMetricFamilyNames(t *testing.T) map[string]struct{} {
	t.Helper()
	names := make(map[string]struct{}, alertValidationOIEFamilyCount-resourceTelemetryOIEFamilyCount)
	for _, line := range goldenLines(t, alertValidationMetricAdditionsPath) {
		name, _, ok := strings.Cut(line, "|")
		if !ok || name == "" {
			t.Fatalf("invalid Prometheus alert validation metric addition %q", line)
		}
		names[name] = struct{}{}
	}
	return names
...
func resourceTelemetryExcludeMetricFamilies(t *testing.T, families []*dto.MetricFamily) []*dto.MetricFamily {
	t.Helper()
	excluded := resourceTelemetryMetricFamilyNames(t)
	for name := range alertValidationMetricFamilyNames(t) {
	...
	for _, family := range families {
		if _, resourceTelemetry := excluded[family.GetName()]; !resourceTelemetry {
...
func alertValidationExcludeMetricFamilies(t *testing.T, families []*dto.MetricFamily) []*dto.MetricFamily {
	t.Helper()
	excluded := alertValidationMetricFamilyNames(t)
	for name := range laterMetricFamilyNames() {
		excluded[name] = struct{}{}
	}
	filtered := make([]*dto.MetricFamily, 0, len(families))
	for _, family := range families {
		if _, alertValidation := excluded[family.GetName()]; !alertValidation {
			filtered = append(filtered, family)
		}
	}
	return filtered
...
func TestDataIntegrityPrometheusEndToEndMetricContract(t *testing.T) {
	families, events := dataIntegrityE2EFixture(t)

	seenIdentities := make(map[string]struct{}, 128)
	for _, family := range families {
		for _, metric := range family.Metric {
			identity := family.GetName() + "{" + dataIntegrityMetricLabels(metric) + "}"
			if _, duplicate := seenIdentities[identity]; duplicate {
				t.Fatalf("duplicate emitted Prometheus metric identity %s", identity)
			}
			seenIdentities[identity] = struct{}{}
			if value, ok := dataIntegrityMetricValue(metric); ok && (math.IsNaN(value) || math.IsInf(value, 0)) {
				t.Fatalf("non-finite emitted metric %s=%v", identity, value)
			}
		}
	}

	got := normalizeHistoricalMetricHelp(dataIntegritySelectedMetricContract(t, families))
	wantBytes, err := os.ReadFile("testdata/data-integrity-prometheus-e2e-contract.golden")
	if err != nil {
		t.Fatal(err)
	}
	want := normalizeHistoricalMetricHelp(string(wantBytes))
	if got != want {
		t.Fatalf("selected end-to-end metric contract changed\nwant:\n%s\ngot:\n%s", want, got)
	}

	gotSchema := normalizeHistoricalMetricHelp(dataIntegrityEmittedMetricSchemaContract(resourceTelemetryExcludeMetricFamilies(t, families)))
	wantSchemaBytes, err := os.ReadFile("testdata/data-integrity-prometheus-emitted-schema-contract.golden")
	if err != nil {
		t.Fatal(err)
	}
	wantSchema := normalizeHistoricalMetricHelp(string(wantSchemaBytes))
	...
	event := events[0]
	_, instanceUUID := dataIntegrityE2EUUID()
	if event.tag != "BEHAVIOR" || event.event != "behavior_alert" ||
		event.domain != "instance-00000042" || event.instanceUUID != instanceUUID ||
		event.projectUUID != "project-data-integrity" || event.projectName != "Data Integrity Project" || event.userUUID != "user-data-integrity" {
...
func dataIntegrityFullRegistryFixture(t *testing.T) ([]*dto.MetricFamily, map[string]struct{}) {
	t.Helper()
	...
	_, instanceUUID := dataIntegrityE2EUUID()
	const (
		domainName  = "instance-data-integrity-full-registry"
		serverName  = "data-integrity-full-registry"
		projectUUID = "project-full-registry"
		projectName = "Data Integrity Full Registry"
		userUUID    = "user-full-registry"
		vmIP        = "10.42.0.10"
		remoteIP    = "198.51.100.99"
		ifname      = "tap-data-integrity"
		volumeUUID  = "volume-data-integrity"
		diskPath    = "vda"
	)
	...
	mc.tm.hostThreatHits = map[string]map[string]string{
		"DATA_INTEGRITY": {remoteIP: "ipv4"},
	}

	mc.im.domainMeta[instanceUUID] = &DomainStatic{
		ConfiguredVCPUs: 1, ConfiguredMemMB: 4096,
		ConfiguredInterfaces: []Interface{{}}, PortUUIDs: []string{"port-test"}, PortIPsByUUID: map[string][]IP{"port-test": {{Address: vmIP, Family: "ipv4"}}},
		Name:            serverName,
		InstanceUUID:    instanceUUID,
		UserUUID:        userUUID,
		UserName:        "fixture-user",
	...
	mc.im.netSamples[shard][instanceUUID] = netSample{
		rxPkts: rxPkts - 100, txPkts: txPkts - 75, rxDrop: rxDrop - 2, txDrop: txDrop - 1,
		interfaceSet: resourceNetworkIdentity(mc.im.domainMeta[instanceUUID], deduplicateIPs(mc.im.domainMeta[instanceUUID].FixedIPs)), ts: sampleAt,
	}
	...
	for _, provider := range mc.tm.Providers {
		connAgg.ProviderSourcesIncluded[provider.Name] = struct{}{}
	...
	agg := &hostAgg{projects: make(map[string]struct{})}
	uuid, _ := dataIntegrityE2EUUID()
	record := libvirt.DomainStatsRecord{
		Dom:    libvirt.Domain{Name: domainName, UUID: uuid},
		Params: params,
	}
	mc.collectDomainMetrics(record, connAgg, nil, agg, 100000, true, true, true)
	mc.volumeRetypeJobs[volumeRetypeKey{InstanceUUID: instanceUUID, DiskPath: "vdb"}] = volumeRetypeJob{
		Domain:                record.Dom,
		...
		ProjectName:           projectName,
		UserUUID:              userUUID,
		VolumeUUID:            "volume-d2d5547f-8b38-42e3-95ba-40e720dc4259",
		DiskType:              "volumes",
		DiskPath:              "vdb",
		DestinationVolumeUUID: "volume-b885bfa5-bb3f-47b7-bd49-3980fae73ce8",
		DestinationDiskType:   "premium",
		SourceName:            "volumes/volume-d2d5547f-8b38-42e3-95ba-40e720dc4259",
		DestinationName:       "premium/volume-b885bfa5-bb3f-47b7-bd49-3980fae73ce8",
		Ready:                 true,
		ReadyObservedAt:       now.Add(-30 * time.Second),
		ProgressPercent:       10.295297,
		ProgressAvailable:     true,
		ObservationAttempted:  true,
		ObservationHealthy:    true,
		ObservedStartAt:       now.Add(-time.Minute),
		ConfirmedAt:           now,
	}
	completedRetype := mc.volumeRetypeJobs[volumeRetypeKey{InstanceUUID: instanceUUID, DiskPath: "vdb"}]
	completedRetype.DiskPath = "vdc"
	completedRetype.ProgressAvailable = false
	mc.volumeRetypeCompleted[volumeRetypeCompletionKeyForJob(completedRetype)] = volumeRetypeCompletion{
		Job:         completedRetype,
		Result:      "success",
		CompletedAt: now,
	}
	mc.volumeRetypeResults = volumeRetypeResultCounters{Success: 1, Unsuccessful: 1, Unknown: 1}
	...
	close(metricCh)
	metrics := mc.inventoryMetricBatch([]libvirt.DomainStatsRecord{record}, mc.im.domainMeta)
	for metric := range metricCh {
		metrics = append(metrics, metric)
	}

	registry := prometheus.NewRegistry()
	registry.MustRegister(dataIntegrityE2EMetricCollector{metrics: metrics})
...
func TestDataIntegrityFullPrometheusRegistrySchemaContract(t *testing.T) {
	...
	got := normalizeHistoricalMetricHelp(dataIntegrityEmittedMetricSchemaContract(resourceTelemetryExcludeMetricFamilies(t, contractFamilies)))
	wantBytes, err := os.ReadFile("testdata/data-integrity-prometheus-full-registry-schema.golden")
	if err != nil {
		t.Fatal(err)
	}
...
func TestResourceTelemetryFullPrometheusRegistrySchemaContract(t *testing.T) {
	families, descriptorNames := dataIntegrityFullRegistryFixture(t)
	...
	got := normalizeHistoricalMetricHelp(dataIntegrityEmittedMetricSchemaContract(alertValidationExcludeMetricFamilies(t, contractFamilies)))
	wantBytes, err := os.ReadFile("testdata/resource-telemetry-prometheus-full-registry-schema.golden")
	if err != nil {
		t.Fatal(err)
	}
	want := string(wantBytes)
	if got != want {
		t.Fatalf("Resource telemetry full Prometheus registry schema changed\nexporter_descriptors=%d registry_families=%d\nwant:\n%s\ngot:\n%s", len(descriptorNames), len(contractFamilies), want, got)
	}
}
