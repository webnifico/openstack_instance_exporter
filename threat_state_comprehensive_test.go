...
func TestThreatIntelHistoryInitializationUpdateAndAvailability(t *testing.T) {
	...
	if got := mc.updateIntelHistory("vm-1", 0.8, 100); got != 0.8 {
		t.Fatalf("initial history=%v, want 0.8", got)
	}
	want := 0.8 + ewmaAlpha(15, defaultThreatEWMATau.Seconds())*(0.2-0.8)
	if got := mc.updateIntelHistory("vm-1", 0.2, 115); got < want-0.000001 || got > want+0.000001 {
		t.Fatalf("updated history=%v, want %v", got, want)
	}
	if got, ok := mc.snapshotIntelHistoryAvailable("vm-1"); !ok || got < want-0.000001 || got > want+0.000001 {
		t.Fatalf("history snapshot=(%v,%v), want approximately (%v,true)", got, ok, want)
	}
...
func TestThreatMetricDescriptionAndHostMetricCollection(t *testing.T) {
	...
	if len(descCh) != 14 {
		t.Fatalf("combined descriptor count=%d, want 14 including feed-state family", len(descCh))
	}

	metrics := make([]prometheus.Metric, 0)
	tm.collectHostThreatMetrics(&metrics)
	if len(metrics) != 14 {
		t.Fatalf("host threat metric count=%d, want 14 including five feed-state samples", len(metrics))
	}
...
func TestThreatDomainSignalCollectionFreshStaleAndOutage(t *testing.T) {
	...
	agg := &ConntrackAgg{
		VMIndex:                map[VMIPIdentity]uint32{{InstanceUUID: "vm-1", IP: IPStrToKey(vmIP)}: 0},
		ObservationUnix:        now.Unix(),
	...
	signal, available := mc.collectDomainThreatSignals(agg, map[string]struct{}{vmIP: {}},
		"domain", "server", "vm-1", "project", "project-name", "user", true, true, true, &metrics)
	...
	metrics = nil
	previous, available := mc.collectDomainThreatSignals(agg, map[string]struct{}{vmIP: {}},
		"domain", "server", "vm-1", "project", "project-name", "user", false, false, true, &metrics)
	if !available || previous <= 0 || len(metrics) != 4 {
		t.Fatalf("conntrack outage signal=(%v,%v), metrics=%d", previous, available, len(metrics))
	}
	if got, ok := mc.collectDomainThreatSignals(nil, map[string]struct{}{vmIP: {}},
		"domain", "server", "vm-1", "project", "project-name", "user", false, false, true, &metrics); ok || got != 0 {
		t.Fatalf("unavailable conntrack returned (%v,%v), want unavailable", got, ok)
	}

	p.LastSuccess = float64(now.Add(-3 * time.Hour).Unix())
	tm.spamLastSuccessUnix = float64(now.Add(-3 * time.Hour).Unix())
	if got, ok := mc.collectDomainThreatSignals(agg, nil, "domain", "server", "new-vm", "project", "project-name", "user", true, true, true, &metrics); ok || got != 0 {
		t.Fatalf("stale threat feeds returned (%v,%v), want unavailable", got, ok)
	}
	if got, ok := (&MetricsCollector{}).collectDomainThreatSignals(agg, nil, "domain", "server", "vm", "project", "project-name", "user", true, true, true, &metrics); ok || got != 0 {
...
func TestThreatActiveFlowMetricIncludesCappedEvidence(t *testing.T) {
	...
	if signal != 0.2 {
		t.Fatalf("combined threat signal=%v, want conservative retained-plus-overflow lower bound 0.2", signal)
	}
}
...
func TestThreatLoggingEntryPointsRespectDirectionAndThrottle(t *testing.T) {
	...
	if len(tm.threatLastHit) != 4 {
		t.Fatalf("threat event throttle entries=%d, want 4", len(tm.threatLastHit))
	}
}

func TestBehaviorEventsBypassThreatManagerThrottle(t *testing.T) {
	...
	if len(tm.threatLastHit) != 0 {
		t.Fatalf("behavior lifecycle events entered the threat throttle: got %d entries, want 0", len(tm.threatLastHit))
	}
	if got := strings.Count(logs.String(), `"msg":"behavior_alert"`); got != 2 {
		t.Fatalf("behavior lifecycle events logged=%d, want 2", got)
	}

	tm.logThreatEvent(
		"POLICY", "policy_alert", "domain", "vm", "project", "project-name", "user",
		"kind", "policy_kind",
	)
	tm.logThreatEvent(
		"POLICY", "policy_alert", "domain", "vm", "project", "project-name", "user",
	...
	if len(tm.threatLastHit) != 1 {
		t.Fatalf("policy events did not retain threat throttle ownership: got %d entries, want 1", len(tm.threatLastHit))
	}
}
