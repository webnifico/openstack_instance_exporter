...
type dataIntegrityOwnedLogCaptureHandler struct {
	next slog.Handler
}

func (handler dataIntegrityOwnedLogCaptureHandler) Enabled(ctx context.Context, level slog.Level) bool {
	return handler.next.Enabled(ctx, level)
}

func (handler dataIntegrityOwnedLogCaptureHandler) Handle(ctx context.Context, record slog.Record) error {
	// Some collector tests intentionally close a raw shutdown channel without
	// waiting for the background goroutine's final lifecycle log. That message
	// belongs to the preceding collector test, not to a behavior/threat event
	// contract that installs this global capture immediately afterward.
	if record.Message == "background_collector_shutdown" {
		return nil
	}
	return handler.next.Handle(ctx, record)
}

func (handler dataIntegrityOwnedLogCaptureHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
	return dataIntegrityOwnedLogCaptureHandler{next: handler.next.WithAttrs(attrs)}
}

func (handler dataIntegrityOwnedLogCaptureHandler) WithGroup(name string) slog.Handler {
	return dataIntegrityOwnedLogCaptureHandler{next: handler.next.WithGroup(name)}
}

func dataIntegrityBehaviorManager() *ConntrackManager {
	cm := newBehaviorStateTestManager()
	cm.behaviorOutboundPortNames = builtinBehaviorOutboundMonitoredPorts()
	return cm
}

func dataIntegrityGenericScanStats() *behaviorStats {
	return dataIntegrityGenericScanStatsForPort(22)
}

func dataIntegrityGenericScanStatsForPort(port uint16) *behaviorStats {
...
func dataIntegrityVerticalScanStats() *behaviorStats {
	stats := newBehaviorStats(false)
...
func dataIntegrityAnalyzeBehavior(cm *ConntrackManager, stats *behaviorStats, instanceUUID, serverName string) float64 {
	return dataIntegrityAnalyzeBehaviorWithContext(cm, stats, instanceUUID, serverName, BehaviorContext{})
}

func dataIntegrityAnalyzeBehaviorWithContext(cm *ConntrackManager, stats *behaviorStats, instanceUUID, serverName string, ctx BehaviorContext) float64 {
...
func dataIntegrityThreatManager() *ThreatManager {
...
func resetDataIntegrityBehaviorRuleLogState(t *testing.T) {
	t.Helper()
...
func dataIntegrityKVMap(kvpairs []interface{}) map[string]interface{} {
...
func captureDataIntegrityStructuredLogs(t *testing.T) *bytes.Buffer {
	t.Helper()
	oldRoot := getRootLogger()
	oldDefault := slog.Default()
	buf := &bytes.Buffer{}
	logger := slog.New(dataIntegrityOwnedLogCaptureHandler{
...
func decodeDataIntegrityStructuredLogs(t *testing.T, raw string) []map[string]interface{} {
	t.Helper()
...
func findDataIntegrityStructuredEvent(t *testing.T, events []map[string]interface{}, msg, kind string) map[string]interface{} {
	t.Helper()
...
func assertDataIntegrityStructuredValues(t *testing.T, event map[string]interface{}, want map[string]interface{}) {
	t.Helper()
...
func assertDataIntegrityStructuredTypes(t *testing.T, event map[string]interface{}, want map[string]string) {
	t.Helper()
	if gotType := dataIntegrityJSONType(event["time"]); gotType != "string" {
	...
	for key, wantType := range want {
		...
		if gotType := dataIntegrityJSONType(value); gotType != wantType {
			t.Fatalf("structured field %q type=%s value=%v, want %s; event=%#v", key, gotType, value, wantType, event)
		}
	}
}

func dataIntegrityJSONType(value interface{}) string {
	switch value.(type) {
	case string:
		return "string"
	case json.Number:
...
func dataIntegrityBehaviorAlertRequiredTypes(mining bool) map[string]string {
...
func dataIntegrityBehaviorSummaryRequiredTypes() map[string]string {
