...
func newBehaviorStateTestManager() *ConntrackManager {
	observationBase := time.Now().Truncate(time.Second)
	var observationSequence atomic.Int64
	cm := &ConntrackManager{
	...
	cm.conntrackNowOverride = func() time.Time {
		sequence := observationSequence.Add(1) - 1
		return observationBase.Add(time.Duration(sequence*behaviorPersistenceReferenceSeconds) * time.Second)
	}
