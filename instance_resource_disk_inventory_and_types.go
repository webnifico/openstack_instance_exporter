...
func (mc *MetricsCollector) collectDomainDiskMetrics(
	...
	diskCount, ioSignal, activity, available, _, _, _ := mc.collectDomainDiskMetricsWithSources(
		meta,
		stat,
		now,
	...
	return diskCount, ioSignal, activity, available
}

func (mc *MetricsCollector) collectDomainDiskMetricsWithSources(
	meta *DomainStatic,
	stat *ParsedStats,
	now time.Time,
	domain, serverName, instanceUUID, projectUUID, projectName, userUUID string,
	telemetryFresh bool,
	dynamicMetrics *[]prometheus.Metric,
) (int, float64, float64, bool, string, []string, bool) {
	...
	resourceSources := make([]string, 0, len(stat.Disks)*2)
	...
	for _, blk := range stat.Disks {
		...
		if ok {
			switch {
			case dMeta.SourceFile != "":
				diskType = "local"
				volumeUUID = dMeta.SourceFile
			case dMeta.SourceName != "":
				diskType, volumeUUID = parseDiskType(dMeta.SourceName)
			case dMeta.SourceDev != "":
				diskType = "block"
				volumeUUID = dMeta.SourceDev
			case dMeta.SourceVolume != "":
				diskType = "volume"
				volumeUUID = strings.Trim(strings.TrimSpace(dMeta.SourcePool)+"/"+strings.TrimSpace(dMeta.SourceVolume), "/")
			default:
				diskType = "unknown"
				volumeUUID = "unknown"
			}
		...
		if rwDerivedValid {
			resourceSources = append(resourceSources, "rw:"+key)
		}
		if flushDerivedValid {
			resourceSources = append(resourceSources, "flush:"+key)
		}
		...
		counters := []struct {
			...
			{blk.RdBytesPresent, float64(blk.RdBytes) * bytesToGigabytes, mc.im.instanceDiskReadGbytesTotalDesc},
			{blk.WrBytesPresent, float64(blk.WrBytes) * bytesToGigabytes, mc.im.instanceDiskWriteGbytesTotalDesc},
		...
		if blk.AllocationPresent {
			diskMetrics = append(diskMetrics, prometheus.MustNewConstMetric(mc.im.instanceDiskAllocationBytesDesc, prometheus.GaugeValue, float64(blk.Allocation), domain, serverName, instanceUUID, projectUUID, projectName, userUUID, volumeUUID, diskType, diskPath))
		}
	...
	authoritativeDiskSet := telemetryFresh &&
		len(diskMetaMap) == len(meta.Disks) &&
		len(stat.Disks) == len(seenDiskPaths) &&
		len(diskMetaMap) == len(seenDiskPaths)
	if stat.BlockCountPresent {
		authoritativeDiskSet = authoritativeDiskSet &&
			stat.BlockCount == uint64(len(stat.Disks)) &&
			stat.BlockCount == uint64(len(seenDiskPaths))
	}
	if authoritativeDiskSet {
		for diskPath := range diskMetaMap {
			if _, seen := seenDiskPaths[diskPath]; !seen {
				authoritativeDiskSet = false
				break
			}
		}
	}
	if authoritativeDiskSet {
		mc.im.pruneDiskSamples(instanceUUID, seenDisks)
	}
	...
	completeSet := authoritativeDiskSet && len(stat.Disks) == len(seenDiskPaths)
	if stat.BlockCountPresent {
	...
	identityKeys := make([]string, 0, len(seenDisks))
	for key := range seenDisks {
		identityKeys = append(identityKeys, key)
	}
	sort.Strings(identityKeys)
	sort.Strings(resourceSources)
	return diskCountDomain, maxDiskIOSignal, maxDiskActivity, completeSet, strings.Join(identityKeys, "\x00"), resourceSources, authoritativeDiskSet
}
