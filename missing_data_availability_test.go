...
func TestStaleThreatFeedAndMissingConntrackAreUnavailable(t *testing.T) {
	...
	score, available := mc.collectDomainThreatSignals(nil, nil, "domain", "server", "vm", "project", "name", "user", false, false, true, &metrics)
...
func TestThreatSeverityIsUnavailableDuringInitialReadOutage(t *testing.T) {
	now := time.Now()
	...
	score, available := mc.collectDomainThreatSignals(nil, nil, "domain", "server", "vm", "project", "name", "user", false, false, true, &metrics)
	if available || score != 0 || len(metrics) != 0 {
		t.Fatalf("missing conntrack snapshot reported healthy: score=%v available=%v metrics=%d", score, available, len(metrics))
	}
}

func TestAttentionAvailabilityRequiresEveryConfiguredWeightedInput(t *testing.T) {
	...
	if attentionInputsAvailable(cfg, true, false, false) {
		t.Fatal("attention score renormalized around missing configured inputs")
	}
	if !attentionInputsAvailable(cfg, true, true, true) {
		t.Fatal("complete configured attention evidence was ignored")
	}
	...
	if !attentionInputsAvailable(zeroWeight, false, true, false) {
		t.Fatal("the only positively weighted input was ignored")
	}
}
