...
* Opens a libvirt connection for the cycle.
* Enumerates active domains.
* Builds and refreshes `InstanceMeta` (UUIDs, project/user info, IPs, disks).
* Collects per-instance CPU, memory, disk, and network stats.
* Reads conntrack once and aggregates flows per fixed IP.
* Runs behavior analysis and threat intel matching.
* Computes per-instance resource, threat, and attention scores.
* Updates a snapshot in memory.
...
* Libvirt lifecycle is owned by the collector for each cycle.
